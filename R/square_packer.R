#' Data Generation for Square Packing
#' @description
#'
#' A tool for creating a data frame of values that create a square packing design when plotted.
#'
#' @param n The total number of squares you would like the function to attempt to create. A single numeric value.
...
#' @param big_r The radius used for your 'big' sized squares A single numeric value.
#' @param med_r The radius used for your 'medium' sized squares. A single numeric value.
#' @param small_r The radius used for your 'small' sized squares. A single numeric value.
#' @param color_pal A vector of hex color codes that will be mapped to the data.
#' @param color_type Default is "regular" - The colors will be mapped in order from big squares to small squares. "reverse" - The colors will be mapped in reversed order from small to big squares. "random" - The colors will be mapped randomly to any sized square.
#' @param angles Default is `FALSE`. `TRUE` turns on rotations for the squares.
#'
#' @return A Tibble
#' @export
#'
...
square_packer <- function(n, min_x = 0, max_x = 100, min_y = 0, max_y = 100,
  ...
  if(angles){
    big_squares <- pmap(list(big_x,
                             big_y,
                             new_iter,
                             big_angles), ~artpack::rotator(tibble(x = c(-sq_coord,sq_coord,sq_coord,-sq_coord,-sq_coord) + ..1,
                                                                   y = c(-sq_coord,-sq_coord,sq_coord,sq_coord,-sq_coord) + ..2,
                                                                   group = paste0("big_",..3)), ..4)) |>
      list_rbind()
  } else{
    big_squares <-  pmap(list(big_x,
                              big_y,
                              new_iter), ~tibble(x = c(-sq_coord,sq_coord,sq_coord,-sq_coord,-sq_coord) + ..1,
                                                 y = c(-sq_coord,-sq_coord,sq_coord,sq_coord,-sq_coord) + ..2,
                                                 group = paste0("big_",..3))) |>
      list_rbind()
  }
  ...
  if(angles){
    med_squares <- pmap(list(med_x,
                             med_y,
                             new_iter,
                             med_angles), ~artpack::rotator(tibble(x = c(-sq_coord,sq_coord,sq_coord,-sq_coord,-sq_coord) + ..1,
                                                                   y = c(-sq_coord,-sq_coord,sq_coord,sq_coord,-sq_coord) + ..2,
                                                                   group = paste0("med_",..3)), ..4)) |>
      list_rbind()
  } else{
    med_squares <-  pmap(list(med_x,
                              med_y,
  ...
  if(angles){
    small_squares <-  pmap(list(small_x,
                                small_y,
                                new_iter,
                                small_angles), ~artpack::rotator(tibble(x = c(-sq_coord,sq_coord,sq_coord,-sq_coord,-sq_coord) + ..1,
                                                                        y = c(-sq_coord,-sq_coord,sq_coord,sq_coord,-sq_coord) + ..2,
                                                                        group = paste0("small_",..3)), ..4)) |>
      list_rbind()
  } else{
    small_squares <-  pmap(list(small_x,
                                small_y,
